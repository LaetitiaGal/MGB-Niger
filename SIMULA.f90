	...
	OPEN(FILPRP,FILE=OUTPUT_DIRECTORY // 'Qorigin.PRP',STATUS='UNKNOWN')
	OPEN(FILHID,FILE=OUTPUT_DIRECTORY // 'Qoutlet.QCL',STATUS='UNKNOWN')
	OPEN(FILTUD,FILE=OUTPUT_DIRECTORY // 'QTmini_NC.QBI',STATUS='UNKNOWN',RECL=NC,FORM='UNFORMATTED',ACCESS='DIRECT')
	OPEN(FILBAC,FILE=OUTPUT_DIRECTORY // 'WB_IB15.TXT',STATUS='UNKNOWN')
	!OPEN(FILBAC1,FILE=OUTPUT_DIRECTORY // 'aWC_NB.WBI',STATUS='UNKNOWN',RECL=NB,FORM='UNFORMATTED',ACCESS='DIRECT')
	!OPEN(FILBAC2,FILE=OUTPUT_DIRECTORY // 'aP_NB.WBI',STATUS='UNKNOWN',RECL=NB,FORM='UNFORMATTED',ACCESS='DIRECT')
	!OPEN(FILBAC3,FILE=OUTPUT_DIRECTORY // 'aINT_NB.WBI',STATUS='UNKNOWN',RECL=NB,FORM='UNFORMATTED',ACCESS='DIRECT')
	!OPEN(FILBAC4,FILE=OUTPUT_DIRECTORY // 'aET_NB.WBI',STATUS='UNKNOWN',RECL=NB,FORM='UNFORMATTED',ACCESS='DIRECT')
	!OPEN(FILBAC5,FILE=OUTPUT_DIRECTORY // 'aQsurf_NB.WBI',STATUS='UNKNOWN',RECL=NB,FORM='UNFORMATTED',ACCESS='DIRECT')
	!OPEN(FILBAC6,FILE=OUTPUT_DIRECTORY // 'aQint_NB.WBI',STATUS='UNKNOWN',RECL=NB,FORM='UNFORMATTED',ACCESS='DIRECT')
	!OPEN(FILBAC7,FILE=OUTPUT_DIRECTORY // 'aQbas_NB.WBI',STATUS='UNKNOWN',RECL=NB,FORM='UNFORMATTED',ACCESS='DIRECT')
...
!	
	DO IT=1,NT
		...
		! record special characteristics for sub-basin
		!do ib=1,NB		!subbasin
		ib=15
		...
		!enddo
		! Water Balance variables by Watershed (=22)
		!write (1976, rec=it)(wbm(ib,it),ib=1,nb) ! average in-soil water
		!write (filbac2, rec=it)(pm2(ib,it),ib=1,nb) ! average precip
		!write (filbac3, rec=it)(sim(ib,it),ib=1,nb) ! average interception
		!write (filbac4, rec=it)(em(ib,it),ib=1,nb)	! average evapotranspiration
		!write (filbac5, rec=it)(dsum(ib,it),ib=1,nb) ! average surface flow
		!write (filbac6, rec=it)(dinm(ib,it),ib=1,nb) ! average subsurface flow
		!write (filbac7, rec=it)(dbam(ib,it),ib=1,nb) ! average groundwater flow
  ...
  ! 
    ...
    DO K=1,NUMHIDG
        ...
        if(hdFLAG0>1)then 
            ...
            strARQUIVO = 'SIM_Q_'//TRIM(adjustl((strIHIDG)))//'.TXT'
        ...
        OPEN(801,FILE=OUTPUT_DIRECTORY // ''//strARQUIVO,STATUS='UNKNOWN',ACTION='WRITE')
            WRITE(801,*)'   DAY   MONTH   YEAR        Q_(m3/s)'
            DO IT=1,NT
...
!        CLOSE(801)
    ...
    DO K=1,NUMHIDG
        WRITE(strIHIDG,'(I10)')IHIDG(K)
        strARQUIVO = 'SIM_Q_Hfl_'//TRIM(adjustl((strIHIDG)))//'.TXT'
        OPEN(801,FILE=OUTPUT_DIRECTORY // ''//strARQUIVO,STATUS='UNKNOWN',ACTION='WRITE')
		WRITE(801,*)'   DAY   MONTH   YEAR  HOUR        Q_(m)'
           DO IT=1,NT
